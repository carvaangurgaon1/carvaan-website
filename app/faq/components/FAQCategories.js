...
/* ----------------------------------------------
   PAGE METADATA (fixed viewport placement)
---------------------------------------------- */
...
export default function FAQCategories({ faqs = [] }) {
  ...
  // ✅ Safe reduce (no crash if faqs is undefined or empty)
  ...
  // ✅ Fallback when no FAQs available
  if (!faqs.length) {
    return (
      <section className="max-w-6xl mx-auto mt-10 px-4 text-center text-gray-500">
  ...
  return (
    <section className="max-w-6xl mx-auto mt-10 px-4">
      {Object.entries(grouped).map(([category, items]) => (
        <div
          key={category}
          className="bg-white shadow rounded-lg border mb-6"
        >
          {/* Category Header */}
